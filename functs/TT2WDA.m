function TTDA = TT2WDA(train_data, train_label, test_data,test_label, I, tau, para_App,KNN)
...
k   = length(ord{1});
% k = computeK(I,2);
% mD{1}=1:k; mD{2}= k+1:length(I);
% ord{1}=I(mD{1}); ord{2}=I(mD{2});
...
% Initialize Ui
Ui = U2Ui_tau(reshape(train_data, [ord{1}, N*Rs]), tau); % U_1, ....,U_k, A 
Ui = Ui(1:k); 
rln=size(Ui{k},3);
LSub         = reshape(merge_tensor(Ui(1:k)),[],rln);
lprojTrain=permute(tmprod(train_data,LSub',1),[4,2,3,1]);
Ui(k+1:n+1) = U2Ui_tau(reshape(lprojTrain,[ord{2},N*rln]), tau); % U_1, ....,U_k, A 
rrn=size(Ui{n},3);

nT= size(test_data,2);
test_data=reshape(test_data,[I,nT]);
test_data=permute(test_data,[mD{1},n+1,mD{2}]);
test_data=reshape(test_data, [Ls,nT,Rs]);
...
for iter =1:para_App.maxiterL
    ...
    test_proj=ndim_unfold(tmprod(test_data,{LSub',RSub'},[1,3]),2)';
